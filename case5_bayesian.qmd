...
mcmc.pars <- list(iter = 50000, burn = 2000, nchain=4, thin=1)
...
ess_table <- data.frame(
  Parameter = names(ess),
  ESS = round(as.numeric(ess), 2)
)

kable(ess_table, 
      caption = "Effective Sample Size (ESS) for All Parameters",
      align = "lc")
...
# Convert jagsUI output to a single MCMC matrix
sim <- as.mcmc(jags.out)               # gives an mcmc.list
sim_mat <- do.call(rbind, sim)         # <-- FIX: binds chains into one matrix
df <- as.data.frame(sim_mat)           # now safe to convert to data frame

# OPTIONAL: remove random effects if you want only fixed effects
# df <- df %>% select(starts_with("beta"), alpha)
...
# Forest plot
ggplot(post_summary,
       aes(x = reorder(parameter, mean), y = mean)) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = low95, ymax = high95), width = 0.15) +
  coord_flip() +
  theme_minimal() +
  labs(
...
sim_mcmc <- mcmc(sim_mat)

params <- c("beta_age", "beta_notify", "beta_transport", 
            "beta_TPA", "beta_Thromb")

autocorr.plot(sim_mcmc[, params])

```

```{r}
traceplot(jags.out, 
          parameters = c("beta_age", "beta_notify", 
                         "beta_transport", "beta_TPA", "beta_Thromb"))
```
...
df <- as.data.frame(sim_mat)

# We scale the prior SD to match the posterior scale.
# This keeps the shape of the prior but makes it visible.
make_prior_posterior <- function(param) {
  post <- df[[param]]
  
  # prior SD set to 4 × posterior SD (good balance for visualization)
  prior_sd <- sd(post) * 4
  
  prior <- rnorm(length(post), mean = 0, sd = prior_sd)
  
  tibble(
    value = c(prior, post),
    distribution = rep(c("Prior (scaled)", "Posterior"), each = length(post)),
    parameter = param
  )
}

# Combine all parameters
plot_df <- bind_rows(lapply(params, make_prior_posterior))
ggplot(plot_df, aes(x = value, fill = distribution)) +
  geom_density(alpha = 0.45) +
  facet_wrap(~parameter, scales = "free") +
  theme_minimal() +
  labs(
    title = "Posterior vs Prior Distributions (Scaled Prior)",
    x = "Parameter Value",
    y = "Density"
  ) +
  scale_fill_manual(values = c("skyblue", "gray40"))

```

```{r}
invlogit <- function(x) 1 / (1 + exp(-x))
time_params <- paste0("beta_time[",1:8,"]")

time_summary <- df %>%
  select(all_of(time_params)) %>%
...
pred_df <- time_summary %>%
  mutate(
    prob_mean = invlogit(mean),
    prob_low  = invlogit(low95),
    prob_high = invlogit(high95)
  )
ggplot(pred_df, aes(x = time, y = prob_mean)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = prob_low, ymax = prob_high), width = 0.1) +
  theme_minimal() +
  labs(title = "Predicted Probability of Good Outcome Over Study Period",
       x = "Time (period)",
...
df %>%
  pivot_longer(cols = c(beta_age, beta_notify, beta_transport,
                        beta_TPA, beta_Thromb),
