model {
  ...
  # Transport | Race, Site, Z
  for (i in 1:N){
    Transport[i] ~ dbern(pi_T[i])
    logit(pi_T[i]) <- gamma0
                      + gamma_site[Site[i]]
                      + gamma_Thromb * hadThrombectomy[i]
                      + gamma_TPA * hadTPA[i]
                      + gamma_thrComp * thrComplic[i]
                      + gamma_tpaComp * tpaComplic[i]
  }
  gamma0 ~ dnorm(0, 0.001)
  for (s in 1:9){ gamma_site[s] ~ dnorm(0, 0.001) }
  gamma_Thromb ~ dnorm(0, 0.001)
  gamma_TPA ~ dnorm(0, 0.001)
  gamma_thrComp ~ dnorm(0, 0.001)
  gamma_tpaComp ~ dnorm(0, 0.001)
  
  # Notify | Transport, Race, Site, Z
  ...
  for (q in 1:8){ delta_time[q] ~ dnorm(0, 0.001) }
  
  # Age | Notify, Transport, Race, Site, Z
  for (i in 1:N){
  ...
  for (q in 1:8){ eta_time[q] ~ dnorm(0, 0.001) }
