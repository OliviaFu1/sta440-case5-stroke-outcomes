...
We fit the logistic mixed-effects model using the `glmer` function from the `lme4` package, with the bobyqa optimizer to improve convergence. The imputations were highly stable across iterations. We examined the binned residual plots for all 20 imputations and found no substantial systematic deviations or concerning residual patterns. For illustration, we present the plot from imputation 1 (Figure \@ref(fig:freq1)). To evaluate predictive performance, we computed the mean AUC across all 20 imputed models, getting an average of 0.743 (Table 2). We also present the ROC curve for imputation 1 (Figure \@ref(fig:freq2)), which indicates a model with high sensitivity but relatively weaker specificity, so the model identifies good outcomes more reliably than poor ones.
...
```{r label = "freq1", fig.cap = "Binned Residual Plot for Imputation 1", fig.show = 'hold'}
...
```{r label = "freq2", fig.cap = "Outcomes change over time", fig.show = 'hold'}
roc_obj <- roc(y, p1)
