...
```{r label = "eda4", echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Age Distribution by Site 1-9", fig.show = 'hold'}
ggplot(x, aes(x = Age)) +
...
```{r label = "eda5", echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Distribution by Site", fig.show = 'hold'}
...
```{r label = "eda6", fig.cap = "Patient discharge outcomes by treatment and pre-hospital factors", fig.show = 'hold'}
...
```{r label = "eda7", fig.cap = "Patient discharge outcomes by demographic factors", fig.show = 'hold', fig.width=6, fig.length=6}
x$AgeGroup <- cut(
  x$Age,
...
```{r label = "eda8", fig.cap = "Outcomes change over time", fig.show = 'hold'}
x |>
...
```{r}
est_list  <- lapply(models$analyses, fixef)      # length m, each a named numeric vector
...
# 2. Within-imputation variance (mean of SE^2)
