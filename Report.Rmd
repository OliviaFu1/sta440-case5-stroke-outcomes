...
To assess the missingness mechanism, we examined patterns across sites, time, and outcomes. Missingness was clearly non-uniform across sites (e.g., Site 170 had no Age values) and varied over time (@fig-eda3), with Age and PreHospNotify heavily missing early in the baseline period before improving in later quarters (@fig-eda2). Missingness also differed by outcome, particularly for Age and PreHospNotify (@fig-eda4). These patterns rule out MCAR and support a MAR assumption. Because the missingness is unlikely to depend on the unobserved values themselves, we proceeded under a Missing at Random (MAR) assumption. We also explored variation in demographic and clinical characteristics across sites and their relationships with discharge outcomes. Age distributions differed across centers, with some sites treating more older patients (@fig-eda5), and racial composition also varied, with certain hospitals serving predominantly Caucasian patients and others having higher proportions of African American or Other race groups (@fig-eda6). On the other hand, gender, pre-hospital notification, and arrival mode were relatively consistent across sites.
...
From the pooled regression results (Table 1), older age substantially reduced the odds of a good outcome ($\beta$ = –0.041, p \< 0.001). EMS arrival, thrombectomy, and both TPA- and thrombectomy-related complications also lowered the odds, reflecting greater stroke severity. Age was modeled linearly, though nonlinear effects may exist, like incorporating age × treatment interactions may help clarify. Female gender and African American race were similarly associated with worse outcomes. In contrast, TPA treatment significantly improved the odds of home/rehab discharge ( $\beta$= 0.570, p \< 0.001). Time-period effects were modest: the middle period was similar to baseline, and the end period showed a small, non-significant improvement ($\beta$ = 0.240, p = 0.175). This suggests a possible, but not statistically strong, improvement as the program became fully implemented, potentially limited by site variability and the COVID period.

## **Bayesian Model**

### **Rationale and Implementation**

For the Bayesian analysis, we modeled homeOrRehab as Bernoulli with a logit link, with fixed effects for site (9 centers), time (8 quarters), Age, Gender, Race, EMS transport, Pre-hospital notification, TPA, thrombectomy, and their complications. We tested alternative age specifications (log-age, age\^2, centering), but none improved convergence, so we retained a linear age term. We also tried modeling site as a random effect, but it did not improve fit or mixing, so we kept the fixed-effects structure. All eight time periods were included to preserve quarter-level variation. Missing-data variables, Race, Transport, Pre-hospital notification, and Age, were modeled jointly. Race (3 categories) used a multinomial model with site-specific probabilities and Dirichlet(0.5) priors. Transport was modeled with a logistic regression including site and key clinical predictors. Pre-hospital notification was modeled as a logistic function for EMS arrivals only, enforcing its structural dependence on EMS transport. Age was modeled as normal with a mean varying by site, race, and time. Predictors in each submodel were chosen based on regressions of missingness indicators and reasonable drivers of missingness.
...
We used standard Bayesian diagnostics for assesment, including trace plots, effective sample sizes (ESS), autocorrelation functions (ACF), and Gelman–Rubin $\hat{R}$ statistics. Trace plots for the primary regression coefficients (age, EMS transport, thrombectomy, tPA treatment, and treatment-related complications) showed tight, overlapping chains with no visible drift, indicating good mixing. The one exception was the age coefficient ($\beta{age}$), which mixed more slowly and had a lower ESS (Table 3). We tested several transformations (log-age, age\^2, centering), but all produced similar posteriors. Despite the reduced ESS, $\beta{age}$ remained well-identified with a narrow posterior, so its interpretation is stable. ESS values for all other fixed-effect parameters were extremely high (often \>150,000), indicating strong chain independence (Table 3). ACF plots supported this: most coefficients showed rapid autocorrelation decay, again with $\beta{age}$ as the only modest outlier, but still acceptable for inference (@fig-bay4).

Gelman–Rubin $\hat{R}$ values for the outcome-model parameters were all approximately 1.00, with upper bounds below 1.03 (@fig-bay2). Some parameters in the missing-data submodels (some delta_time and site_gamma effects) had higher $\hat{R}$ values and greater uncertainty, which is expected given weak identifiability of missingness mechanisms and does not affect inference for the outcome model. Posterior–prior comparisons (@fig-bay3) showed that the main clinical coefficients have posteriors substantially narrower than the diffuse Normal priors, indicating that the data were informative. In contrast, gender, race, site, and time effects remained wide and weakly identified, consistent with their high variability in the data.

### Results

When evaluating associations, we examined posterior means and 95% credible intervals (Table 4). Age had a negative effect on the log-odds of a good outcome (–0.065, 95% CrI: –0.079, –0.051). EMS transport (–1.266, –1.965 to –0.589), thrombectomy (–0.582, –0.918 to –0.246), and both TPA and thrombectomy complications (–1.377, –2.173 to –0.591) also showed clearly negative effects. These intervals exclude 0, indicating strong evidence that these factors reduce the probability of home/rehab discharge. By contrast, TPA treatment showed a positive association (0.594, 95% CrI: 0.247, 0.944), making it the only treatment variable with a credible interval fully above 0, consistent with improved outcomes. To address whether outcomes improved over time, we examined the posterior distributions for the eight quarterly time effects. All time coefficients had credible intervals that included 0, and their posterior means were small in magnitude. The posterior probability that the final quarter had higher odds of a good outcome than the baseline quarter was below 0.75, providing little evidence of systematic improvement. Site effects displayed similar uncertainty, with all site-level credible intervals overlapping 0. Together, these results indicate clear associations for key demographic and clinical variables but provide weak evidence that the quality-improvement program produced measurable improvements in discharge outcomes over the study period.

# Limitations, Future Directions, and Conclusion

Our Bayesian model had several important limitations. Some posterior distributions showed substantial uncertainty, and key variables, PreHospNotify, EMSTransport, Race, and Age, had high levels of missingness. Measurement inconsistencies across sites and quarters may also have introduced noise. Because of large site-to-site variability, time effects were estimated imprecisely. Future work should consider a more flexible imputation structure and alternative prior specifications to improve identifiability.

Overall, younger age and TPA treatment were clearly associated with better outcomes, whereas EMS transport, thrombectomy, and treatment-related complications were associated with worse outcomes, reflecting greater underlying severity. We found no strong evidence of systematic improvement in outcomes over the study period at either the site or time level, and the uncertainty around these effects suggests that centers may benefit from re-evaluating their relative and aggregate performance in stroke outcome management.
...
beta_params <- varnames(sim)[grep("^beta", varnames(sim))]

n <- length(beta_params)
nrow <- ceiling(n / 4)
ncol <- 4
