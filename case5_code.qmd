...
x$miss_PreHospNotify <- ifelse(is.na(x$PreHospNotify), 1, 0)
x$miss_Race2         <- ifelse(is.na(x$Race2), 1, 0)
x$miss_EMSvsCar      <- ifelse(is.na(x$EMSvsCar), 1, 0)
x$miss_homeOrRehab   <- ifelse(is.na(x$homeOrRehab), 1, 0)

table(x$miss_Age, x$Gender) |> chisq.test()
table(x$miss_Age, x$Race2)  |> chisq.test()
table(x$miss_Age, x$siteID) |> chisq.test()
```
...
full_x<-complete(missing_removed)
```
