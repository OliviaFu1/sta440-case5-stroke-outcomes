...
missing_removed<-mice(x,m=5,method= "pmm",seed=2025)
missing_removed_data<-complete(missing_removed)
...
```{r}
linear_model_missing<-glm(homeOrRehab~., missing_removed_data,family=binomial)
...
missing_removed_data <- missing_removed_data |>
  mutate(Age_c = Age - mean(Age, na.rm = TRUE))

model_mixed <- glmer(homeOrRehab ~ EMSvsCar + PreHospNotify + hadThrombectomy +
                    hadTPA + tpaComplic + thrComplic + Age_c + period + Gender +
                    (1 | siteID),
                    data = missing_removed_data, family = binomial)

model_mixed2 <- glmer(homeOrRehab ~ EMSvsCar + PreHospNotify + hadThrombectomy +
                    hadTPA + tpaComplic + thrComplic + Age_c + period + Gender + Race2 +
                    (1 | siteID),
                    data = missing_removed_data, family = binomial)

summary(model_mixed)
anova(model_mixed, model_mixed2)
```

```{r}
missing_removed_data$resid <- residuals(model_mixed, type = "pearson")
missing_removed_data$fitted <- fitted(model_mixed)

ggplot(missing_removed_data, aes(x = fitted, y = resid)) +
